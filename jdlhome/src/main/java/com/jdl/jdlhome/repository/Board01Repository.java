...
public interface Board01Repository extends JpaRepository<Board01, Long> {
    @Query(
            ...
            //value = "select a.*, b.name, Count(*) OVER() as tot_cnt from board01 a inner join user b where a.user_id = b.user_id order by a.no desc limit ?1 offset ?2"
            value = "select a.*, Count(*) OVER() as tot_cnt from board01 a order by a.no desc limit ?1 offset ?2"
            , nativeQuery = true
    )
